...
for ROInum in [100,200, 300, 400, 500]:
    ...
    for fn in subInfo['id']:
        path = '/media/user/4TB/matlab/PET_center/Rest2/FC_reg4/par' + str(ROInum) + '/' + fn + '.mat'
                ...
                for step, (b_x, b_y) in enumerate(train_loader):  # gives batch data
                    ...
                    if tr_accuracy>0.75 and epoch>50:
                        ...
                        if  auc >= auc_baseline and sens >0.70 and spec >0.70 and tr_accuracy>0.75:
                            auc_baseline = auc
                            torch.save(model.state_dict(),'./models_MCI2SCD/gcn_'+str(neg)+'vs'+str(pos)+'_cv'+str(cv)+'_'+str(ROInum)+'.pth')
...
for cv in [1,2,3,4]:
    ...
    model = MyModels.GCN_base(ROInum=ROInum)
    model.load_state_dict(torch.load('./models_MCI2SCD/gcn_'+str(neg)+'vs'+str(pos)+'_cv'+str(cv)+'_'+str(ROInum)+'.pth'))
    model.cuda()
    predicted_all = []
    test_y_all = []
    model.eval()
...
scio.savemat('./results/amci_namci_'+str(ROInum)+'.mat', {'qualified':qualified})
