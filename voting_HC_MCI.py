...
for cv in [1,2,3,4]:
    ...
    for ROInum in range(100,ROI+100,100):
        ...
        model.load_state_dict(
            torch.load('./models_MCI2SCD/gcn_' + str(neg) + 'vs_rest_cv' + str(cv) + '_' + str(ROInum) + '.pth'))
...
scio.savemat('./results/hc_mci_multi.mat', {'qualified':qualified})

# deep feature comparison (SCD)
neg=0

subInfo = pd.read_csv('./SCD-MCI-subinfo_revise_final_2.csv')
subInfo = subInfo[(subInfo['grouping']==3)]
...
for cv in [1,2,3,4]:
    ...
    for ROInum in range(100,ROI+100,100):
        ...
        subcount = 0
        for fn in subInfo['id']:
            path = '/media/PJLAB\liumianxin/18675978328/Shanghaitech/matlab/PET_center/Rest2/FC_reg4/par' + str(
                ROInum) + '/' + fn + '*.mat'
            path = glob.glob(path)
            temp = scio.loadmat(path[0])
            FC_all[subcount, :, :] = temp['FC']
...
HC = (np.array(predicted_all) == 0).sum()/ float(len(predicted_all))
MCI = (np.array(predicted_all) == 1).sum()/ float(len(predicted_all))

print('|HC:', HC,
      '|MCI:', MCI
      )
      
scio.savemat('./results/hc_mci_'+str(ROInum)+'_subtying_SCD.mat', {'predicted_all':predicted_all})
